...
# REVISI: Import tambahan metrik RMSE & MAE sesuai standar silabus bootcamp
...
def load_and_train_all():
    ...
    for name, (model, pre) in models.items():
        ...
        # Hitung metrik jujur (skala Rupiah asli balik dari Log)
        y_pred_log = pipe.predict(X_test)
        y_test_real = np.expm1(y_test)
        y_pred_real = np.expm1(y_pred_log)
        
        score = r2_score(y_test_real, y_pred_real)
        # REVISI: Hitung nilai RMSE dan MAE murni dari deviasi harga asli pasar
...
with tab1:
    ...
    # Rekap Anal常规 Outlier IQR Data Asli (Fitur Numerik)
...
with tab3:
    ...
    # REVISI: Format tampilan tabel agar RMSE & MAE rapi ber-comma nominal Rupiah asli
    df_display = df_results.copy()
    df_display["R²"] = df_display["R²"].map(lambda x: f"{x:.4f}")
    df_display["RMSE"] = df_display["RMSE"].map(lambda x: f"Rp {x:,.0f}")
    df_display["MAE"] = df_display["MAE"].map(lambda x: f"Rp {x:,.0f}")
    ...
    # Hitung nilai persentase meleset absolut untuk fitur baru
    pct_error = (np.abs(y_aktual - y_prediksi) / y_aktual) * 100
    
    # Pengelompokan Kategori Toleransi Akurasi
    ...
    with col_res3:
        ...
        # REVISI FIX: ax=fig_res3.gca() diganti jadi ax=ax_res3 biar sinkron di server online
        sns.barplot(data=df_toleransi, x='Kategori', y='Persentase', ax=ax_res3, palette=['#2ca02c', '#ff7f0e', '#d62728'])
        sns.despine(left=True, bottom=True)
